...
void ConfigurePedInteraction(Ped ped) {
    ...
    if (!ENTITY::IS_ENTITY_DEAD(ped) && !PED::IS_PED_DEAD_OR_DYING(ped, true)) {
        ...
        // Fuga e Reações
        PED::SET_PED_CONFIG_FLAG(ped, 442, true);  // O ped não foge de uma interação
        ...
        // PED::SET_PED_CONFIG_FLAG(ped, 155, true);  // PCF_EnableCompanionAIAnalysis (habilita IA de análise)
