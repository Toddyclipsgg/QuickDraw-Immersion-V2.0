...
void ButtonMonitoring()
{
    if (submenu == Closed) // Se o menu estiver fechado
    {
        // Verifica se o botão para abrir o menu foi pressionado
    ...
    else
    {
        ...
        // Verifica se a opção de rolar para cima foi pressionada
        if (PAD::IS_CONTROL_JUST_PRESSED(0, joaat("INPUT_FRONTEND_UP")))
        {
            if (currentOption == 1) // Se a opção atual é a primeira
            {
                // Muda para a última opção
                currentOption = optionCount;
                currentMenuMaxOptions = optionCount;
                if (optionCount > maxOptions)
                    currentMenuMinOptions = optionCount - maxOptions + 1; // Ajusta a contagem mínima
                else
                    currentMenuMinOptions = 1; // Se não, reseta para 1
            }
            else
            {
                currentOption--; // Decrementa a opção atual
                // Ajusta as contagens mínima e máxima de opções se necessário
                if (currentOption < currentMenuMinOptions) {
                    currentMenuMinOptions = currentOption;
                    currentMenuMaxOptions = currentOption + maxOptions - 1;
                }
        ...
        // Verifica se a opção de rolar para baixo foi pressionada
        if (PAD::IS_CONTROL_JUST_PRESSED(0, joaat("INPUT_FRONTEND_DOWN")))
        {
            if (currentOption == optionCount) // Se a opção atual é a última
            {
                // Muda para a primeira opção
                currentOption = 1;
                currentMenuMinOptions = 1;
            ...
            {
                currentOption++; // Incrementa a opção atual
                // Ajusta as contagens mínima e máxima de opções se necessário
                if (currentOption > currentMenuMaxOptions) {
                    currentMenuMaxOptions = currentOption;
                    currentMenuMinOptions = currentOption - maxOptions + 1;
        ...
        // Verifica se a opção de rolar para a esquerda foi pressionada
        ...
        // Verifica se a opção de rolar para a direita foi pressionada
        ...
        // Verifica se a tecla esquerda está pressionada continuamente
        if (PAD::IS_DISABLED_CONTROL_PRESSED(0, joaat("INPUT_FRONTEND_LEFT"))) {
            fastLeftPress = true; // Marca que a tecla esquerda está pressionada rapidamente
        }
        // Verifica se a tecla direita está pressionada continuamente
        if (PAD::IS_DISABLED_CONTROL_PRESSED(0, joaat("INPUT_FRONTEND_RIGHT"))) {
            fastRightPress = true; // Marca que a tecla direita está pressionada rapidamente
        }
    }
}
*/
