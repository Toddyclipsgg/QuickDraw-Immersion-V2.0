...
// Declara��o de vari�veis globais para log e console
...
// Fun��o para logar informa��es no arquivo
void logToFile(const std::string& message) {
    std::lock_guard<std::mutex> guard(logMutex); // Garante que as threads n�o colidam
    if (isLoggingEnabled && logFile.is_open()) {
        logFile << message << std::endl;
    }
}

// Fun��o para exibir no console
...
// Fun��o para controlar o log e console
...
// Fun��o para logar imediatamente
...
// Fun��o para rodar o logger em uma thread separada
void startLogThread() {
    logThread = std::thread([]() {
        while (true) {
            ...
            // Aqui voc� pode adicionar outras opera��es a serem feitas periodicamente
        }
        });
}

// Fun��o para parar a thread de log
void stopLogThread() {
